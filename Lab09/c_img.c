...
void create_img(struct rgb_img **im, size_t height, size_t width){
    *im = (struct rgb_img *)malloc(sizeof(struct rgb_img));
    (*im)->height = height;
    (*im)->width = width;
    printf("malloc size for raster is %d\n", 3*height*width);
    (*im)->raster = (uint8_t *)malloc(3 * height * width);
    printf("pixel 1: %d %d %d",((*im)->raster)[0],((*im)->raster)[1],((*im)->raster)[2]);
    printf("pixel 2: %d %d %d",((*im)->raster)[3],((*im)->raster)[4],((*im)->raster)[5]);
}
